...
public class BookTypeController {
	...
	public String add(@Validated BookType bookType, BindingResult br, Model model, HttpServletRequest request) {
		...
		try {
			...
			request.setAttribute("message", java.net.URLEncoder.encode("图书类型添加成功!", "UTF-8"));
			return "message";
		} catch (Exception e) {
			e.printStackTrace();
			request.setAttribute("error", java.net.URLEncoder.encode("图书类型添加失败!"));
			return "error";
		}
	...
	public String update(@Validated BookType bookType, BindingResult br, Model model, HttpServletRequest request)
		...
		try {
			...
			e.printStackTrace();
			request.setAttribute("error", java.net.URLEncoder.encode("图书类型添加失败!", "UTF-8"));
			return "error";
		}
	}
