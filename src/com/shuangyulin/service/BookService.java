...
public class BookService {
	...
	/*查询当前页Book信息*/
	public List<Book> queryBook(String bookName,BookType bookType,String barcode,String publishDate,int currentPage) throws Exception{
    	String where = "where 1=1";
    	if(!bookName.equals("")) {
    		where = where + " and t_book.bookName like '%" + bookName + "%'";
    	}
    	
    	Boolean b = null != bookType && bookType.getBookTypeId() != null && bookType.getBookTypeId()!=0; 
    	if(b) {
    		where += " and t_book.bookType=" + bookType.getBookTypeId();
    	}
    	
    	if(!barcode.equals("")) {
    		where = where + " and t_book.barcode like '%" + barcode + "%'";
    	}
    	
    	if(!publishDate.equals("")) {
    		where = where + " and t_book.publishDate like '%" + publishDate + "%'"; 
    	}
